...
public class IndicatorRender extends ComponentRenderer<Indicator> {
    ...
    public void render(
        ...
        int totalLight = LightTexture.packLight(
                Math.max(ownBrightness, LightTexture.getLightBlock(packedLightIn)),
                Math.max(ownBrightness, LightTexture.getLightSky(packedLightIn))
        );
