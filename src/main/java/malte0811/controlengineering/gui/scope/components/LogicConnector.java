...
public class LogicConnector implements IScopeComponent {
    ...
    public void render(GuiGraphics graphics) {
        if (connectedLine == DigitalModule.NO_LINE) {
            return;
        }
        ScreenUtils.fill(graphics.pose(), area.minX(), area.minY(), area.maxX(), area.maxY(), 0xff << 24);
        graphics.pose().pushPose();
        graphics.pose().translate(area.minX() + 1, area.center().y(), 0);
        TransformUtil.shear(graphics.pose(), -1 / 5f, 0);
        ScreenUtils.fill(graphics.pose(), 0, 0, 17, 100, WIRE_COLOR_DARK);
        final int numLines = 8;
        final double lineWidth = 17 / (double) numLines;
        for (int i = 0; i < numLines; ++i) {
            final var startLine = lineWidth * i + lineWidth / 4;
            ScreenUtils.fill(graphics.pose(), startLine, 0, startLine + lineWidth / 2, 100, WIRE_COLOR_LIGHT);
        }
