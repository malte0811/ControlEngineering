...
public class CRTDisplay {
    ...
    public void draw(GuiGraphics graphics, @Nullable TraceId hovered) {
        ScreenUtils.setupScissorMCScaled(xMin, yMin, width, height);
        graphics.pose().pushPose();
        graphics.pose().translate(xMin, yMin, 0);
        graphics.pose().scale(divWidth, divHeight, 1);
        drawBackground(graphics);
        for (final var trace : menu.getTraces().traces()) {
            drawTrace(graphics, trace, trace.getTraceId().equals(hovered));
        }
    ...
    private void drawBackground(GuiGraphics graphics) {
        graphics.fill(0, 0, NUM_HORIZONTAL_DIVS, NUM_VERTICAL_DIVS, BACKGROUND);
        for (int divH = 0; divH <= NUM_HORIZONTAL_DIVS; ++divH) {
            drawVerticalDivider(graphics.pose(), divH, DIVIDER_COLOR);
        }
        for (int divV = 0; divV <= NUM_VERTICAL_DIVS; ++divV) {
            drawHorizontalDivider(graphics.pose(), divV, DIVIDER_COLOR);
        }
        drawHorizontalDivider(graphics.pose(), NUM_VERTICAL_DIVS / 2, CENTER_DIVIDER_COLOR);
        drawVerticalDivider(graphics.pose(), NUM_HORIZONTAL_DIVS / 2, CENTER_DIVIDER_COLOR);
    }
