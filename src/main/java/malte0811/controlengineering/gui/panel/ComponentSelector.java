...
public class ComponentSelector extends NestedWidget {
    ...
    private void renderAvailableType(
            @Nonnull GuiGraphics graphics, @Nullable PanelComponentType<?, ?> type, int x, int y, boolean highlight
    ) {
        ...
        if (type != null) {
            ...
            drawCenteredShrunkString(graphics, mc.font, name, 0, colWidth, 0, 0);

            graphics.pose().translate(0, mc.font.lineHeight, 0);
            renderComponentInGui(graphics, type, colWidth, actualRowHeight - mc.font.lineHeight * 1.5);
        }
    ...
    public static void renderComponentInGui(
            @Nonnull GuiGraphics graphics, @Nonnull PanelComponentType<?, ?> type, double width, double height
    ) {
        graphics.pose().pushPose();
        ...
        graphics.pose().scale(extraScale, extraScale, .01f);
        graphics.pose().translate(-component.getSize(level).x() / 2f, -component.getSize(level).y() / 2f, 0);
    ...
    private static void drawCenteredShrunkString(
            GuiGraphics graphics, Font font, String text, int xMin, int xMax, int y, int color
    ) {
