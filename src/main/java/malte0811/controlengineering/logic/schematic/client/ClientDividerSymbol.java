...
public class ClientDividerSymbol extends ClientCellSymbol<Integer> {
    ...
    @Override
    public void renderCustom(PoseStack transform, int x, int y, Integer rLower) {
        super.renderCustom(transform, x, y, rLower);
        var font = Minecraft.getInstance().font;
        transform.pushPose();
        final var scale = 5f;
        transform.translate(5.25 + x, 2 + y + scale / 7.5, 0);
        transform.scale(1 / scale, 1 / scale, 1);
        font.draw(transform, Integer.toString(VoltageDivider.TOTAL_RESISTANCE - rLower), 0, 0, 0);
        transform.translate(0, 4 * scale, 0);
        font.draw(transform, Integer.toString(rLower), 0, 0, 0);
        transform.popPose();
    }
}
