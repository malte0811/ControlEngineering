...
public class ClientIOSymbol extends ClientSymbol<BusSignalRef, IOSymbol> {
    ...
    @Override
    protected void renderCustom(GuiGraphics graphics, int x, int y, @Nullable BusSignalRef state, int alpha) {
        int color;
        if (state != null) {
        ...
        TextUtil.renderBoxWithText(graphics, color, text, 4, blockX, y, 3, 3);
    }
