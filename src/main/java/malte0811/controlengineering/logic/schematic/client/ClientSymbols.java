...
public class ClientSymbols {
    ...
    public static <State> void renderCenteredInBox(
            SymbolInstance<State> inst, GuiGraphics graphics, int x, int y, int xSpace, int ySpace
    ) {
        ...
        graphics.pose().translate(x + xSpace / 2., y + ySpace / 2., 0);
        graphics.pose().scale(scale, scale, 1);
        graphics.pose().translate(-width / 2., -height / 2., 0);
        render(inst, graphics, 0, 0, 0xff);
        graphics.pose().popPose();
    }

    public static <State> void render(SymbolInstance<State> inst, GuiGraphics graphics, int x, int y, int alpha) {
        render(inst.getType(), graphics, x, y, inst.getCurrentState(), alpha);
    }

    public static <State>
    void render(SchematicSymbol<State> serverSymbol, GuiGraphics graphics, int x, int y, State state, int alpha) {
        getClientSymbol(serverSymbol).render(graphics, x, y, state, alpha);
    }

    public static void render(PlacedSymbol placed, GuiGraphics graphics, int alpha) {
        render(placed.symbol(), graphics, placed.position().x(), placed.position().y(), alpha);
    }

    public static void render(Schematic schematic, GuiGraphics graphics, Vec2d mouse) {
        for (PlacedSymbol s : schematic.getSymbols()) {
            render(s, graphics, 0xff);
        }
        for (SchematicNet net : schematic.getNets()) {
            final int color = net.contains(mouse.floor()) ? SELECTED_WIRE_COLOR : WIRE_COLOR;
            net.render(graphics, color, schematic.getSymbols());
        }
    }
