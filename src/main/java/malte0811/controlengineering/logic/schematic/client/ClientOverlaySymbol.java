...
public class ClientOverlaySymbol<State, Symbol extends SchematicSymbol<State>> extends ClientSymbol<State, Symbol> {
    ...
    @Override
    protected void renderCustom(GuiGraphics graphics, int x, int y, State state, int alpha) {
        original.renderCustom(graphics, x, y, state, alpha);
        graphics.pose().pushPose();
        graphics.pose().translate(x + xOff + 0.5, y + yOff, 0);
        graphics.pose().scale(1 / 6f, 1 / 6f, 1);
        final var font = Minecraft.getInstance().font;
        final var width = font.width(overlay) - 1;
        graphics.drawString(font, overlay, (int) (-width / 2f), 0, alpha << 24, false);
