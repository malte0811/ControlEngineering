...
public abstract class ClientSymbol<State, Symbol extends SchematicSymbol<State>> {
    ...
    protected abstract void renderCustom(GuiGraphics graphics, int x, int y, State state, int alpha);

    public abstract void createInstanceWithUI(Consumer<? super SymbolInstance<State>> onDone, State initialState);

    public final void render(GuiGraphics graphics, int x, int y, State state, int alpha) {
        renderCustom(graphics, x, y, state, alpha);
        for (SymbolPin pin : serverSymbol.getPins(state)) {
            pin.render(graphics.pose(), x, y, SchematicNet.WIRE_COLOR, alpha);
        }
    }
