...
public class SchematicNet {
    ...
    public void render(GuiGraphics graphics, int color, List<PlacedSymbol> symbols) {
        for (WireSegment segment : segments) {
            segment.renderWithoutBlobs(graphics.pose(), color);
        }
        ...
        for (ConnectedPin pin : getOrComputePins(symbols)) {
            pin.render(graphics.pose(), color);
        }
    }
